...
app.innerHTML = `
  <div class="hud">
    <section class="panel">
      ...
      <p class="lead">Separate F2 experiment: Quest passthrough + tracked Touch aiming + holographic targets. F1 VR remains frozen on main.</p>
      <div class="status" id="status"></div>
      <p class="controls">Enter AR/MR · Trigger: fire · Plane detection is optional evidence, not a gate.</p>
    </section>
...
if (!('xr' in navigator)) {
  ...
  }).catch(() => setStatus('ar', 'immersive-ar: support check failed', 'bad'));
}
...
for (let i = 0; i < 2; i++) {
  ...
  controllerStates.push({ controller, grip });
}
  ...
  if (frame && now - lastPlaneStatusUpdate > 500) {
    ...
    try {
      if (frame.detectedPlanes !== undefined) {
        ...
        setStatus('planes', 'Plane detection: not exposed in this session', 'pending');
      }
    } catch {
