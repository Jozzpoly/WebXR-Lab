...
app.innerHTML = `
  <div class="hud">
    <section class="panel">
      ...
      <h1>MR Reactor Spike</h1>
      <p class="lead">Isolated Quest 2 mixed-reality proof: passthrough + local-floor + tracked Touch aiming + holographic targets.</p>
      <div class="status" id="status"></div>
      <p class="controls">Start MR · Trigger: fire · Plane detection is optional evidence, not a gate.</p>
    </section>
...
setStatus('planes', 'Plane detection: optional / waiting');
setStatus('session', 'MR session: idle');

let immersiveArSupported = null;
let currentSession = null;
let maxDetectedPlanes = 0;
...
function makeMrButton() {
  ...
  async function startSession() {
    if (!navigator.xr || immersiveArSupported !== true) return;
    button.disabled = true;
    button.textContent = 'STARTING MR…';
    setStatus('session', 'MR session: requesting immersive-ar…');
...
function spawnBurst(position, color = 0xff78a6) {
  for (let i = 0; i < 10; i++) {
...
renderer.xr.addEventListener('sessionend', () => {
  ...
  setStatus(
    'session',
    `MR session: ended · max detected planes ${maxDetectedPlanes}`,
    immersiveArSupported === false ? 'bad' : 'ok'
  );
});

const clock = new THREE.Clock();
let lastPlaneStatusUpdate = 0;
...
function animate(_time, frame) {
  ...
  if (frame && now - lastPlaneStatusUpdate > 500) {
    lastPlaneStatusUpdate = now;
    try {
      if (frame.detectedPlanes !== undefined) {
        const count = frame.detectedPlanes.size ?? [...frame.detectedPlanes].length;
        maxDetectedPlanes = Math.max(maxDetectedPlanes, count);
        setStatus('planes', `Plane detection: available · current ${count} · max ${maxDetectedPlanes}`, count > 0 ? 'ok' : 'pending');
      } else {
        setStatus('planes', 'Plane detection: not granted/exposed in this session', 'pending');
      }
    } catch {
      setStatus('planes', 'Plane detection: unavailable / permission not granted', 'pending');
    }
  }
